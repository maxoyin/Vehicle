...
export const VehicleType = (props: IVehicleTypeProps) => {
  ...
  return (
    <div>
      <h2 id="vehicle-type-heading" data-cy="VehicleTypeHeading">
        <Translate contentKey="prospectServiceApp.vehicleType.home.title">Vehicle Types</Translate>
        <div className="d-flex justify-content-end">
          <Button className="mr-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="prospectServiceApp.vehicleType.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to={`${match.url}/new`} className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="prospectServiceApp.vehicleType.home.createLabel">Create new Vehicle Type</Translate>
          </Link>
        </div>
      </h2>
        ...
        {vehicleTypeList && vehicleTypeList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                <th>
                  <Translate contentKey="prospectServiceApp.vehicleType.id">ID</Translate>
                </th>
                <th>
            ...
            <tbody>
              {vehicleTypeList.map((vehicleType, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{vehicleType.displayName}</td>
                  <td>{vehicleType.isDisplayOn ? 'true' : 'false'}</td>
                  <td>{vehicleType.description}</td>
                  <td>
                    {vehicleType.assestClass ? (
                      <Link to={`assest-class/${vehicleType.assestClass.id}`}>{vehicleType.assestClass.id}</Link>
                    ) : (
                      ''
                    )}
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="prospectServiceApp.vehicleType.home.notFound">No Vehicle Types found</Translate>
            </div>
          )
        )}
      </div>
