...
export default class VehicleTypeUpdatePage {
  pageTitle: ElementFinder = element(by.id('prospectServiceApp.vehicleType.home.createOrEditLabel'));
  saveButton: ElementFinder = element(by.id('save-entity'));
  cancelButton: ElementFinder = element(by.id('cancel-save'));
  codeInput: ElementFinder = element(by.css('input#vehicle-type-code'));
  displayNameInput: ElementFinder = element(by.css('input#vehicle-type-displayName'));
  isDisplayOnInput: ElementFinder = element(by.css('input#vehicle-type-isDisplayOn'));
  descriptionInput: ElementFinder = element(by.css('input#vehicle-type-description'));
  assestClassSelect: ElementFinder = element(by.css('select#vehicle-type-assestClass'));
  ...
  async assestClassSelectLastOption() {
    await this.assestClassSelect.all(by.tagName('option')).last().click();
  }

  async assestClassSelectOption(option) {
    await this.assestClassSelect.sendKeys(option);
  }

  getAssestClassSelect() {
    return this.assestClassSelect;
  }

  async getAssestClassSelectedOption() {
    return this.assestClassSelect.element(by.css('option:checked')).getText();
  }
  ...
  async enterData() {
    ...
    await this.assestClassSelectLastOption();
    await this.save();
